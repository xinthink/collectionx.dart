// ignore_for_file: unnecessary_this, can_be_null_after_null_aware
...
extension MapExt<K, V> on Map<K, V> {
  ...
  bool any(EntryPredicate<K, V> test) =>
      this != null ? entries.any((e) => test(e.key, e.value)) : false;

  /// Returns `true` if all entries match the given predicate [test].
  bool all(EntryPredicate<K, V> test) => this?.entries.all((e) => test(e.key, e.value));

  /// Returns `true` if **no** entries match the given predicate [test].
  bool none(EntryPredicate<K, V> test) => this?.entries.none((e) => test(e.key, e.value));
  ...
  Iterable<MapEntry<K, V>> where(EntryPredicate<K, V> test) =>
      this?.entries?.where((e) => test(e.key, e.value));
  ...
  Iterable<T> mapEntries<T>(EntryTransform<K, V, T> f) =>
      this?.entries?.map((e) => f(e.key, e.value));
  ...
  Iterable<T> flatMap<T>(EntryTransform<K, V, Iterable<T>> f) =>
      this?.entries.flatMap((e) => f(e.key, e.value));
